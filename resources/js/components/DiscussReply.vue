<template>
    <div :id="'reply-' + reply.id" class="bg-white p-4 rounded-lg shadow-md mb-4">
      <!-- User Information -->
      <div class="flex items-start mb-4">
        <a :href="`/profile/${reply.user.id}`" class="flex items-center space-x-2">
          <img :src="reply.user.avatar_path" alt="User Avatar" class="w-10 h-10 rounded-full object-cover">
      ...
      <!-- Reply Content -->
      <p class="mt-2 text-gray-700"><b>{{ reply.body }}</b></p>

      <!-- Action Buttons -->
      <div class="flex items-center justify-between mt-4">
        <!-- <div v-if="canEditOrDelete" class="space-x-2"> -->
        <div v-if="true" class="space-x-2">
          <button @click="showEditModal" class="bg-blue-500 text-white px-3 py-1 rounded-lg hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-400">Edit</button>
        ...
        <div class="space-x-2">
          ...
          <button v-if="reply.discussionreplies.length > 0" @click="openAllRepliesModal" class="text-blue-500 hover:underline">View All Replies</button>
        </div>
      </div>


      <EditReplyModal :reply="reply" v-if="showEditModal" @close="showEditModal = false"/>
       <ReplyModal :reply="reply" v-if="showReplyModal" @close="showReplyModal = false"/>
    </div>
  </template>
...
//   const userIsAuthenticated = computed(() => user.isAuthenticated);
  const formattedCreatedAt = computed(() => new Date(props.reply.created_at).toLocaleString('en-US', { dateStyle: 'short', timeStyle: 'short' }));
